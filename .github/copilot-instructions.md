...
- **OIDC + Key Vault flow:** the first jobs of every master workflow run `guard-trigger` → `resolve-oidc` → (per job) `load-secrets`. `resolve-oidc` returns Skyline defaults inside `SkylineCommunications`, caller-provided values otherwise, or `use-oidc=false` (which makes downstream Key Vault loading a no-op). OIDC parameters flow top-down through `with:` to sub-pipelines.
